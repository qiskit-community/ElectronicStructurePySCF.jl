...
"""
function ElectronicStructure.to_pyscf(md::MolecularSpec)
    pymol = _PYSCF.gto.Mole(atom=ElectronicStructure.to_pyscf(md.geometry), basis=md.basis)
    pymol.spin = md.spin
...
"""
PyMol(md::MolecularSpec) = PyMol(ElectronicStructure.to_pyscf(md))
